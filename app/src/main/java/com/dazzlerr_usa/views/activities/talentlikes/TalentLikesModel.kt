...
class TalentLikesModel(val mContext: Context, val mView:TalentLikesView) : TalentLikesPresenter
{
    ...
    override fun getlikedTalents(user_id: String, page: String, isShowProgressbar: Boolean) {

        mView.showProgress(true ,isShowProgressbar )

        val webServices = RetrofitClient.getClient(Constants.baseUrl).create(WebServices::class.java)
        modelsService = webServices.getTalentLikesApi("application/x-www-form-urlencoded" , Constants.auth_key,user_id  ,page
        ...
        APIHelper.enqueueWithRetry(modelsService ,Constants.RETRY_COUNT ,object : Callback<ModelsPojo>
        {
            override fun onResponse(call: Call<ModelsPojo>, response: Response<ModelsPojo>)
            {
                try
                {
                    ...
                    Timber.e("TalentLikes response: "+ responseStr)

                    if(response.body()?.status!!)
                        mView.onGetLikesSuccess(response.body() as ModelsPojo)
